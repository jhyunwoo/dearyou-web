    ...
    <ProtectedPage>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 overflow-y-auto m-5 p-3 border-4 border-slate-100 rounded-2xl">
          ...
          <Link className="m-2 p-1 text-xl flex border-2 border-gray-500" href={'/chats/' + data?.id} key={key}>
            <span className="p-1 font-bold text-blue-800">{
              data?.expand['buyer']?.id === user.id ? (data?.expand['seller']?.name) : (data?.expand['buyer']?.name)}</span>
